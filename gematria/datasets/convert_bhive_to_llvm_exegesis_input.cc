...
absl::StatusOr<AnnotatedBlock> AnnotateBasicBlock(
  ...
  auto addrs = GetAccessedAddrs(*bytes, exegesis_annotator, 50);
          ...
          for (int snippet_index = current_batch_start_index;
            ...
            std::shared_future<absl::StatusOr<AnnotatedBlock>>
                current_snippet_future = current_batch_group.async(
                    [&](const std::string& current_hex_value, const int current_snippet_index)
                        -> absl::StatusOr<AnnotatedBlock> {
			absl::StatusOr<AnnotatedBlock> block_to_return = AnnotateBasicBlock(current_hex_value,
                                                bhive_importer,
                                                exegesis_annotator.get(),
                                                *llvm_support, *inst_printer);
			if (current_snippet_index % report_progress_every == 0 && current_snippet_index != 0) {
			  io_mutex.lock();
			  std::cerr << "Just finished annotating block " << current_snippet_index << "\n";
			  io_mutex.unlock();
			}
			return std::move(block_to_return);
                    },
                    basic_blocks[snippet_index], snippet_index);
